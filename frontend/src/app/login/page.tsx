...
function LoginForm() {
  ...
  const setupGoogleIdentity = () => {
    if (typeof window !== "undefined" && window.google?.accounts && googleClientId) {
      try {
        // 1. Initialize GIS ID Token listener
        ...
        // 2. Initialize OAuth2 Token Client for programmatic popup click
          ...
          tokenClientRef.current = window.google.accounts.oauth2.initTokenClient({
            ...
            callback: async (tokenResponse: any) => {
              if (tokenResponse?.error) {
                setErrorMsg(tokenResponse.error_description || "Google authorization cancelled.");
                setOauthLoading(null);
                return;
              }
              if (tokenResponse?.id_token) {
                const { error } = await signInWithGoogleIdToken(tokenResponse.id_token);
                if (error) {
  ...
  // Seamless In-Page Google Popup (Never redirects to Supabase domain)
  const handleGoogleSignIn = async () => {
    ...
    if (googleClientId && typeof window !== "undefined" && window.google?.accounts?.id) {
      ...
      setErrorMsg("Google Sign-In is initializing. Please use your email or wait a moment.");
    }
  };
  ...
  return (
    <>
      {/* Load Google Identity Services script strictly forced to English locale (hl=en) */}
      <Script 
        ...
        >
          ...
          <div className="text-center space-y-2">
            <Link href="/" className="inline-flex items-center gap-2.5">
              <div className="w-10 h-10 rounded-xl bg-[#ff5c47]/10 border border-[#ff5c47]/25 flex items-center justify-center text-[#ff5c47] shadow-sm">
          ...
          <div className="p-8 rounded-2xl bg-[#141517] border border-[#232529] space-y-5 shadow-xl">
            ...
            {/* Mode Switcher Tabs */}
            <div className="flex bg-[#18191c] p-1 rounded-xl border border-[#232529]">
              <button
                type="button"
                onClick={() => {
                  setMode("signin");
                ...
                className={`flex-1 py-1.5 text-xs font-semibold rounded-lg transition-all ${
                  mode === "signin"
                    ? "bg-[#232529] text-[#f0f2f5] shadow-sm"
            ...
            {errorMsg && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                className="p-3 rounded-xl bg-[#ff5c47]/10 border border-[#ff5c47]/30 text-xs text-[#ff5c47] flex items-center gap-2"
              >
            ...
            {successMsg && (
              <motion.div
                initial={{ opacity: 0, y: -6 }}
                animate={{ opacity: 1, y: 0 }}
                className="p-3 rounded-xl bg-[#3ec98a]/10 border border-[#3ec98a]/30 text-xs text-[#3ec98a] flex items-center gap-2"
              >
            ...
            {/* Single Sign-On (OAuth) Buttons - 100% Symmetrical Matching */}
            <div className="space-y-2.5">
              ...
              <button
                ...
                className="w-full h-10 px-4 rounded-xl bg-[#18191c] hover:bg-[#202227] border border-[#232529] hover:border-[#2e3238] text-xs font-medium text-[#f0f2f5] flex items-center justify-center relative transition-all shadow-sm disabled:opacity-50 group cursor-pointer"
              >
                {oauthLoading === "google" ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4 pt-1">
              ...
              <div className="pt-2">
                <Button
                  ...
                  className="w-full h-10 rounded-full bg-[#ff5c47] hover:bg-[#ff5c47]/90 text-white font-medium text-xs shadow-lg shadow-[#ff5c47]/20 flex items-center justify-center gap-2 transition-all cursor-pointer"
                >
