...
function LoginForm() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    // 1. If Google Identity Services is initialized with a client ID, trigger native in-page auth
    if (googleClientId && typeof window !== "undefined" && window.google?.accounts?.id) {
      try {
        window.google.accounts.id.prompt((notification: any) => {
          if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
            // Fall back to standard OAuth popup/redirect if One-Tap was dismissed or suppressed
            fallbackOAuthGoogle();
          }
        });
        return;
    ...
    // 2. Direct Supabase Google OAuth Provider Fallback (skips unnecessary hops)
